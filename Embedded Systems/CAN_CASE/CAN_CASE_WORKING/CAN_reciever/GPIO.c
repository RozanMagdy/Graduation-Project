...
void GPIO_void_SetPinValue(GPIO_TypeDef *port, uint8_t pin, uint8_t state){
...
uint8_t GPIO_uint8_GetPinValue( GPIO_TypeDef * port , uint8_t pin){
uint8_t state;
	state = port->ODR;
return state;
}
...
void Led_Tog(uint16_t ms ,uint8_t i)
{
	uint8_t j=0;
 		...
 		for(j=0 ;j<i ;j++)
		{
			GPIO_void_SetPinValue(GPIOC,13,SET);
			Delay(ms);
			GPIO_void_SetPinValue(GPIOC,13,RESET);
			Delay(ms);
		}
