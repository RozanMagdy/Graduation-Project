...
{
	Dio_PortLevelType ret_val;
	
		volatile  Dio_PortLevelType * Pin_Ptr = NULL_PTR;
	switch(PortId)
	{
				case 0:	Pin_Ptr = &PORTA_IDR;
		break;
		case 1:	Pin_Ptr = &PORTB_IDR;
		break;
		case 2:	Pin_Ptr = &PORTC_IDR;
		break;
	}
	ret_val = *Pin_Ptr;
	return (ret_val);
...
void Dio_WritePort(Dio_PortType PortId, Dio_PortLevelType Level)
{
	...
	switch(PortId)
	{
				case 0:	Port_Ptr = &PORTA_ODR;
		break;
		case 1:	Port_Ptr = &PORTB_ODR;
		break;
		case 2:	Port_Ptr = &PORTC_ODR;
		break;
	}
	*Port_Ptr = Level;
...
{
	volatile  Dio_PortLevelType * Port_Ptr = NULL_PTR;
	volatile  Dio_PortLevelType * ClearPort_Ptr = NULL_PTR;
	Dio_ChannelType pinNumber ;
	boolean error = FALSE;
	...
	if(error == FALSE)
	{
		...
		switch(Dio_PortChannels[ChannelId].Port_Num)
		{
			case 0:	Port_Ptr = &PORTA_BSRR;
			       ClearPort_Ptr = &PORTA_BRR;
		        break;
			case 1:	Port_Ptr = &PORTB_BSRR;
			     ClearPort_Ptr = &PORTB_BRR;
		        break;
			case 2:	Port_Ptr = &PORTC_BSRR;
			     ClearPort_Ptr = &PORTC_BRR;
		        break;
			case 3:	Port_Ptr = &PORTD_BSRR;
			       ClearPort_Ptr = &PORTD_BRR;
		        break;
		}
		pinNumber= Dio_PortChannels[ChannelId].Ch_Num;
		if(STD_HIGH == Level)
		{
			
			/* Write Logic High */
			SET_BIT(*Port_Ptr,pinNumber);
		}
		else if(Level == STD_LOW)
		{
			
			/* Write Logic Low */
			SET_BIT(*ClearPort_Ptr,pinNumber);
...
volatile Dio_PortLevelType * ClearPort_Ptr = NULL_PTR;
	...
	if(FALSE == error)
	{
		...
		if(BIT_IS_SET(*PIN_Ptr,Dio_PortChannels[ChannelId].Ch_Num))
		{
			...
			//SET_BIT(*ClearPort_Ptr,Dio_PortChannels[ChannelId].Ch_Num);
			TOGGLE_BIT(*PIN_Ptr,Dio_PortChannels[ChannelId].Ch_Num);
			output = STD_LOW;
		}
		else
		{
...
{
	volatile  Dio_PortLevelType * Port_Index = NULL_PTR;
	volatile  Dio_PortLevelType * ClearPort_Ptr = NULL_PTR;
	boolean error = FALSE;
	...
	if(error == FALSE)
	{
		...
		{
			case 0:	Port_Index = &PORTA_BSRR;
			       ClearPort_Ptr = &PORTA_BRR;
		        break;
			case 1:	Port_Index = &PORTB_BSRR;
			     ClearPort_Ptr = &PORTB_BRR;
		        break;
			case 2:	Port_Index = &PORTC_BSRR;
			     ClearPort_Ptr = &PORTC_BRR;
		        break;
			case 3:	Port_Index = &PORTD_BSRR;
			       ClearPort_Ptr = &PORTD_BRR;
		        break;
		}
			uint32 my_mask  = (0x00000001);
			 uint8  i;
			for (i=0;i<ChannelGroupIdPtr -> mask;i++)
			{
				my_mask |= (uint32)((0x00000001)<<i); //  get the mask of nedded number of pins 
			}
		if(STD_HIGH == Level)
		{		
			/* Write Logic High */
			*Port_Index |= my_mask << (ChannelGroupIdPtr -> offset);
		}
		else if(Level == STD_LOW)
		{
			
			/* Write Logic Low */
			*ClearPort_Ptr |= my_mask << (ChannelGroupIdPtr -> offset);
...
Dio_PortLevelType Dio_ReadChannelGroup( const Dio_ChannelGroupType* ChannelGroupIdPtr )
{
	...
	if(FALSE == error)
	{
		...
		/* Read the required group */	
			output = *PIN_Ptr;
			output = (output & (my_mask << (ChannelGroupIdPtr -> offset))) >> (ChannelGroupIdPtr -> offset);
